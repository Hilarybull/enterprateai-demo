    ...
    <div className="min-h-screen overflow-x-hidden bg-white font-sans text-slate-900 antialiased">
          ...
          <nav className="hidden items-center gap-6 md:flex">
            {NAV_LINKS.map(l =>
              l.href.startsWith("#")
                ...
                : <Link key={l.label} to={l.href} className="text-sm font-medium text-slate-600 hover:text-brand-600 transition-colors">{l.label}</Link>
            )}
          </nav>

          <div className="hidden items-center gap-3 md:flex">
            <Link to="/login" className="text-sm font-semibold text-slate-600 hover:text-brand-600 transition-colors">Log in</Link>
            <Link to="/login" className="rounded-xl bg-brand-600 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-700 transition-colors">Get Started Free</Link>
          </div>
          ...
          <div className="border-t border-slate-100 bg-white px-4 pb-4 md:hidden">
            {NAV_LINKS.map(l =>
              l.href.startsWith("#")
                ...
                : <Link key={l.label} to={l.href} className="block py-2.5 text-sm font-medium text-slate-700" onClick={() => setMobileMenuOpen(false)}>{l.label}</Link>
            )}
            <div className="mt-3 flex gap-3">
              <Link to="/login" className="flex-1 rounded-xl border border-slate-200 py-2 text-center text-sm font-semibold text-slate-700">Log in</Link>
              <Link to="/login" className="flex-1 rounded-xl bg-brand-600 py-2 text-center text-sm font-semibold text-white">Get Started</Link>
            </div>
          </div>
        )}
      </header>
        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="rounded-2xl bg-slate-900 text-white px-7 py-6 space-y-3">
            ...
            <p className="text-sm leading-relaxed">
              <span className="font-semibold text-rose-400">If your plan is weak, your funding application may struggle.</span>
            </p>
            <p className="text-sm leading-relaxed">
              <span className="font-semibold text-emerald-400">If your plan is clear, your conversation becomes stronger.</span>
            </p>
          </div>
        </div>
      </section>
      ...
      <section id="who" className="py-20 bg-slate-50">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-3 sm:grid-cols-2 mb-8">
            {WHO.map((item, i) => (
              <div key={item} className={`flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm${i === WHO.length - 1 && WHO.length % 2 !== 0 ? " sm:col-span-2" : ""}`}>
                <svg className="h-4 w-4 shrink-0 text-brand-500" viewBox="0 0 24 24" fill="currentColor">
                  <path fillRule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm13.36-1.814a.75.75 0 10-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.14-.094l3.75-5.25z" clipRule="evenodd" />
                </svg>
          ...
          <div className="my-8 grid gap-2 sm:grid-cols-2 max-w-md mx-auto text-left">
            {["Validate your idea.", "Generate your business plan.", "Launch to marketplace.", "Test traction."].map(step => (
              <div key={step} className="flex items-center gap-2 text-sm text-slate-300">
                <svg className="h-4 w-4 shrink-0 text-brand-400" viewBox="0 0 24 24" fill="currentColor">
                  <path fillRule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm13.36-1.814a.75.75 0 10-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.14-.094l3.75-5.25z" clipRule="evenodd" />
                </svg>
                {step}
              </div>
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col items-center justify-between gap-4 sm:flex-row">
            ...
            <div className="flex flex-wrap justify-center gap-5 text-xs text-slate-400">
              <Link to="/legal/privacy" className="hover:text-brand-600 transition-colors">Privacy Policy</Link>
              <Link to="/legal/terms" className="hover:text-brand-600 transition-colors">Terms of Service</Link>
              <Link to="/legal/disclaimer" className="hover:text-brand-600 transition-colors">Disclaimer</Link>
