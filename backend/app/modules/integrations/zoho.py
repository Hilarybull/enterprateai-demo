...
_IMPORT_FIELDS = {
    "Products": "Product_Name,Unit_Price,Product_Category,Description,Currency",
...
def _normalize_imported_product(item: dict, now_iso: str) -> dict:
    ...
    price = float(item.get("Unit_Price") or item.get("base_price") or 0)
...
async def _fetch_org_base_currency(access_token: str) -> str | None:
    ...
    _, api_host = _zoho_hosts()
    # Try org endpoint — returns currency_symbol and locale for the org
    for ver in ("v8", "v3", "v2"):
        url = f"https://{api_host}/crm/{ver}/org"
        try:
